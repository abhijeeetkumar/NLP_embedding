...
def preprocess(file, label_file, is_filter=True):
    ...
    for word, freq in cnt.items():
        # Note that we words whose frequency is less than 2 are excluded. Again,
        # we (incorrectly) use the entire dataset to define the vocabulary),
...
def modeling(args, pos_sentences, neg_sentences, vocab):
    """
    Init the model, train and test each epoch
    """
